---
title: |
  ![](/u/ysundaravadanam/git/providencePipeline/OICR.png){width=1in}
  Providence Therapeutics Draft Report
...
The consensus sequence matches the reference sequence with **`r info$Consensus`** sequencing identity.
\newline
...
Sbjct: **`r params$library`**

``` {r blast_seq, echo=FALSE, engine='bash', comment=''}
cat blast.txt
...
The DNA construct was also sequenced and a consensus sequence was generated from the aligned reads using Bcftools. The consensus reference sequence matches the reference sequence with 100% sequencing identity. Please refer to the **`r params$construct`** report for more information.
\newline
...
get_summary_table<-function(s){
  ...
  s_data<-summary[c("RawReads","AdapterTrimmedReads", "AdapterTrimmedBases","PassedReads", "MappedReads", "MappedPercent", "MeanReadLength", "MeanInsertSize", "InserSizeSD"),,drop=F]
...
library(ggplot2)
#plotgraph<-read.table("read_dist.txt", header=TRUE, sep="\t")
...
```{r error_dist, echo=FALSE, warning=FALSE, out.width = '65%'}
library(ggplot2)
plotgraph<-read.table("depth.txt", header=TRUE, sep="\t")
...
```
The average error rate was **`r params$ErrorPercent`**
...
kable(summary_table, col.names = c('Error', 'Count'), row.names = FALSE,  "latex", booktabs = T)
```
\leavevmode\newline

Error locations greater than 1%:
